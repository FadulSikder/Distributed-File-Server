...
void download(int socketfd, struct sockaddr_in server_address)
{
	// Defining variables
	int check, i=0;
  	char buffer[SIZE], filename[50] = "servfile_download_copy.txt",temp[50]="servfile_download_copy";
  	bzero(buffer, SIZE);
	...
	recv(socketfd, buffer, SIZE, 0);
  	...
  	fprintf(stdout,"\nFile succesfully downloaded from Server!!!!!!!!!!!!!!\nSee the downloaded file '%s' in client folder.\n", temp);
    	fflush(stdout);
...
void delete(int socketfd, struct sockaddr_in server_address)
{
	// Defining variables
  	...
  	fprintf(stdout,"SENDING INSTRACTION TO DELETE FILE");
    	fflush(stdout);
